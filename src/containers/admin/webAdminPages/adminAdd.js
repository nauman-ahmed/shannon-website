...
function AdminAdd(props) {
  ...
  return (
    <div className='mx-5 px-5'>   
        ...
        (props.holder?<img className="mt-1" alt="loading" src={loading} style={{width:"30px"}}/>:<button onClick={(e)=>{props.editUser(e,{id:id,name:name,username:username,email:email,password:password,Role:Role});}} className='mt-4 myBtn active sm float-right px-4' >Update</button>)
        :(props.holder?<img className="mt-1" alt="loading" src={loading} style={{width:"30px"}}/>: <button onClick={(e)=>{props.addUser(e,{name:name,username:username,email:email,password:password,Role:Role});}} className='mt-4 myBtn active sm float-right px-4' >ADD</button>)
